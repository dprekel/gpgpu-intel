...
__attribute__((always_inline)) void
InvPass1(float2* twiddles, uint rw, uint b, uint me, uint inOffset, uint outOffset, __local float *bufInRe, __local float *bufInIm, __global float2 *bufOut, float2 *R0, float2 *R1, float2 *R2, float2 *R3)
{
	...
	if(rw)
	{
	bufOut[outOffset + ( 1*me + 0 + 0 )*1] = (*R0) * 6.2500000000000000e-02f;
	bufOut[outOffset + ( 1*me + 0 + 4 )*1] = (*R1) * 6.2500000000000000e-02f;
	bufOut[outOffset + ( 1*me + 0 + 8 )*1] = (*R2) * 6.2500000000000000e-02f;
	bufOut[outOffset + ( 1*me + 0 + 12 )*1] = (*R3) * 6.2500000000000000e-02f;
	}
...
void fft_back(__constant cb_t *cb __attribute__((max_constant_size(32))), __global float2 * restrict gb)
{
	...
	InvPass0(rw, b, me%4, 0, (me/4)*16, lwb, lds, lds, &R0, &R1, &R2, &R3);
	InvPass1(twiddles, rw, b, me%4, (me/4)*16, 0, lds, lds, lwb, &R0, &R1, &R2, &R3);
}
